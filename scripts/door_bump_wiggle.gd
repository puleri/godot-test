...
func _on_bump_trigger_body_entered(body: Node) -> void:
	if wiggle_in_progress or not _is_player_body(body):
		return
...
func bump_wiggle() -> void:
	...
	tween.tween_property(hinge, "rotation:y", closed_rotation.y + deg_to_rad(first_wiggle_degrees), wiggle_out_time)
	tween.tween_property(hinge, "rotation:y", closed_rotation.y + deg_to_rad(rebound_wiggle_degrees), rebound_time)
	tween.tween_property(hinge, "rotation:y", closed_rotation.y, settle_time)
